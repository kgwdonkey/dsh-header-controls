...
async function main() {
  ...
  // --no-open：dsh web 启动默认会自行打开浏览器；开浏览器的职责统一交给下方 openAfter
  // （按钮流程=no-browser 不开，原标签页轮询自刷新，避免重启后出现两个 dsh web 页面）
  const child = spawn(cfg.node, [bin, cfg.profile, '--no-open', '--host', cfg.host], {
    cwd: cfg.repo,
    detached: true,
    windowsHide: true,
